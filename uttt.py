...
class MiniBoardView(View):
    ...
    def create_buttons(self):
        for i in range(3):
            for j in range(3):
                ...
                if is_won:
                    # self.totalboardswon+=1
                    # if self.totalboardswon==9:
                        # self.clear_items()
    ...
    #     # Calculate who won the most boards
    #     player_wins = {"X": 0, "O": 0}
    #     for board_winner in self.game.global_board:
    #         if board_winner in player_wins:
    #             player_wins[board_winner] += 1

    #     # Determine the winner
    #     if player_wins["X"] > player_wins["O"]:
    #         winner_message = "Player X wins the game!"
    #     elif player_wins["O"] > player_wins["X"]:
    #         winner_message = "Player O wins the game!"
    #     else:
    #         winner_message = "It's a tie!"

    #     # Clear buttons and update the message content
    #     self.clear_items()  # Remove all buttons
    #     asyncio.create_task(self.game.message.edit(
    #         content=f"The game is over! {winner_message}",
    #         view=self
    #     ))
...
class UltimateTicTacToeImage:
    ...
    # Copy the base image
        ...
        font = ImageFont.truetype("ldfcomicsans-font/Ldfcomicsansbold-zgma.ttf", self.cell_size//2)
    ...
    def markboardwon(self,won_board_row,won_board_col,symbol):
        ...
        print(won_board_row)
        draw = ImageDraw.Draw(self.base_image)
        font = ImageFont.truetype("ldfcomicsans-font/Ldfcomicsansbold-zgma.ttf", (self.cell_size)*2.2)
    ...
    def save_board(self, filename="board.png", active_board_idx=None):
        ...
        # image = self.draw_board(active_board_idx)
        if self.image:
...
class UltimateTicTacToe:
    ...
    def make_move(self, board_idx, cell_idx):
        ...
        # if self.next_board is not None and self.next_board>=0 and self.next_board != board_idx:
        #     print(f"You must play on board {self.next_board + 1}!")
        #     return False
